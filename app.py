...
.b-card{background:linear-gradient(135deg,#0f172a,#1e293b);border:1px solid #334155;border-radius:12px;padding:12px;margin-bottom:8px;}
.ce-box{background:#062b20;border-left:5px solid #10b981;border-radius:10px;padding:12px;margin-bottom:8px;}
.pe-box{background:#2d0c13;border-left:5px solid #f43f5e;border-radius:10px;padding:12px;margin-bottom:8px;}
.no-box{background:#241703;border-left:5px solid #f59e0b;border-radius:10px;padding:12px;margin-bottom:8px;}
...
.rw{display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid rgba(255,255,255,0.15);font-size:0.82rem;font-family:monospace;}
...
</style><div class="b-card"><div style="font-size:1.3rem;font-weight:800;">⚡ TradeLense AI Terminal</div><div style="font-size:0.75rem;color:#94a3b8;">Automated Confluence & Clear Entry/Exit Levels</div></div>""", unsafe_allow_html=True)
...
inst = c1.radio("Index", ["NIFTY 50", "BANK NIFTY"], horizontal=True)
...
def get_news():
    ...
    itms, b_w, be_w, sc = [], ['rally', 'gain', 'surge', 'rate cut', 'high'], ['fall', 'plunge', 'slump', 'inflation', 'drop'], 0
    for e in f.entries[:3]:
        t = e.title; p = any(w in t.lower() for w in b_w); n = any(w in t.lower() for w in be_w)
        if p: sc += 1
        if n: sc -= 1
        itms.append(t)
    return ("Bullish (+)" if sc > 0 else ("Bearish (-)" if sc < 0 else "Neutral")), itms

mac, (ns_sent, ns_list) = get_macro(), get_news()
...
with st.expander("📰 Live Breaking Headlines", expanded=False):
    for h in ns_list: st.markdown(f"• {h}")
